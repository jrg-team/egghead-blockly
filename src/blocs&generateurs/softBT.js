...
Blockly.Blocks['soft_bt_init'] = {
  ...
  init: function () {
    ...
    this.setTooltip('调用SoftwareSerial(rxPin, txPin)来创建一个新的SoftwareSerial对象');
  }
};
...
Blockly.Blocks['soft_bt_set_name'] = {
  init: function () {
    this.jsonInit({
      "message0": "%1 蓝牙 设置广播名称 %2",
      "args0": [
        {
          "type": "field_image",
          "src": "media/bt.png",
...
Blockly.Arduino['soft_bt_set_name'] = function (block) {
  var nameInput = this.getFieldValue('BT_NAME_INPUT');
  var code = `  mySerialBT.println("AT+NAME${nameInput || ''}");\n`;
  return code;
};
