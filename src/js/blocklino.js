...
BlocklyDuino.init = function () {
  ...
  BlocklyDuino.workspace.addChangeListener(
...
BlocklyDuino.renderArduinoCodePreview = function () {
  ...
  if (prog != "python") {
    ...
    $("#pre_previewArduino").html(
      prettyPrintOne($("#pre_previewArduino").html(), "py")
    );
  }
};
...
BlocklyDuino.load = function (event) {
  ...
  reader.onloadend = function (event) {
    ...
    document.getElementById("title-project-name").value =
      files[0].name.substring(0, files[0].name.indexOf("."));
...
BlocklyDuino.loadConfig = function () {
  ...
  if (card === undefined) {
    ...
    $("#arduino_card_mini_picture").attr(
      "src",
      profile[BlocklyDuino.selectedCard]["picture"]
    );
    if (
      BlocklyDuino.selectedCard == "nanooptiboot" ||
      BlocklyDuino.selectedCard == "nano" ||
      BlocklyDuino.selectedCard == "nona4809"
    ) {
      $("#warning").show();
    } else {
      $("#warning").hide();
...
BlocklyDuino.change_card = function () {
  ...
  if (window.profile[new_card].cpu != window.profile[card].cpu) {
    if (
      window.confirm(
        MSG["arduino_card"] + window.profile[new_card].description + " ?"
      )
    ) {
      ...
      if (new_prog != "python") {
        ...
        if (
          window.profile[new_card].cpu !=
          window.profile[card]
            .cpu /*&& ((window.profile[new_card].cpu.startsWith("esp")) || (window.profile[card].cpu.startsWith("esp")) ) */
        )
          BlocklyDuino.loadBlocks(mystartfile);
        else BlocklyDuino.loadBlocks();

        BlocklyDuino.loadToolboxDefinition(new_toolbox);
        Blockly.getMainWorkspace().updateToolbox(BlocklyDuino.buildToolbox());
        if (window.localStorage.level == 1) $("#btn_level1").trigger("click");
        else if (window.localStorage.level == 2)
          $("#btn_level2").trigger("click");
        else $("#btn_level3").trigger("click");
...
BlocklyDuino.openConfigToolbox = function () {
  ...
  $("#toolbox")
    ...
    .each(function () {
      ...
      if (n >= 0) {
        ligne =
          '<input type="checkbox" checked="checked" name="checkbox_' +
          i +
          '" id="checkbox_' +
          $(this).attr("id") +
          '"/> ' +
          Blockly.Msg[$(this).attr("id")] +
          "<br/>";
      } else {
        ligne =
...
BlocklyDuino.buildToolboxXML = function (loadIds) {
  loadIds = loadIds.trim();
...
BlocklyDuino.buildToolbox = function () {
  ...
  return BlocklyDuino.buildToolboxXML(loadIds);
};
...
BlocklyDuino.buildToolboxLevel2 = function () {
  ...
  if ($("#defaultCategories2").length) {
    loadIds = $("#defaultCategories2").html().trim();
  }
  window.localStorage.toolboxids = loadIds;
...
BlocklyDuino.buildToolboxLevel3 = function () {
  ...
  if ($("#defaultCategories3").length) {
    loadIds = $("#defaultCategories3").html().trim();
  }
...
BlocklyDuino.workspace_capture = function () {
  ...
  var styleElem = document.createElementNS(
...
BlocklyDuino.saveXmlFile = function () {
  if (window.localStorage.content == "on") {
    ...
    var datenow = Date.now();
    document.getElementById("title-project-name").value === ""
      ? (document.getElementById("title-project-name").value = "Ottocode")
      : null;
    ...
    element.setAttribute(
      "href",
      "data:text/bloc;charset=utf-8," + encodeURIComponent(data)
    );
    element.setAttribute("download", filename);
    element.style.display = "none";
    document.body.appendChild(element);
    ...
    element.setAttribute(
      "href",
      "data:py/ino;charset=utf-8," + encodeURIComponent(code)
    );
    element.setAttribute("download", filename);
    element.style.display = "none";
    document.body.appendChild(element);
